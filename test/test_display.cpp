...
struct Screen {
  ...
  void render(SafetyState st, FaultCode f = FaultCode::NONE,
              uint16_t mask = 0, bool key = true, TimeMs now = 0,
              bool bench = false, const char* play = nullptr,
              uint32_t pos = 0) {
    renderDisplay(buf, st, f, in, mask, key, now, bench, play, pos);
  }
...
TEST(display_always_80_printable_chars) {
  ...
  for (auto st : states) {
    for (int f = 0; f <= 9; ++f) {
      ...
      s.render(st, (FaultCode)f, 0xA5A5, false, 12345, false, "LONGNAME.SHW",
               3600000);
...
TEST(display_boot) {
  ...
  s.render(SafetyState::BOOT_SELFTEST, FaultCode::NONE, 0, true, 0,
           /*bench=*/true);
...
TEST(display_safe_shows_what_is_missing) {
  ...
  s.in.link_ok = false;
  s.render(SafetyState::SAFE);
  EXPECT_TRUE(s.line(0, "SAFE          [MIDI]"));
  EXPECT_TRUE(s.line(1, "NO SIGNAL           "));

  s.in.link_ok = true;
  s.render(SafetyState::SAFE, FaultCode::NONE, 0, /*key=*/false);
...
TEST(display_bench_screens) {
  Screen s;
  s.render(SafetyState::SAFE, FaultCode::NONE, 0, true, 0, /*bench=*/true);
...
TEST(display_armed_mode_bar_and_timing) {
  ...
  s.in.repeat = true;
  s.in.rate = 128;
  s.render(SafetyState::ARMED, FaultCode::NONE, 0x8001);
  ...
  s.in.repeat = false;
  s.in.rate = 5;
  s.render(SafetyState::ARMED, FaultCode::NONE, 0);
  EXPECT_TRUE(s.line(1, "  [................]"));
...
TEST(display_playback_line) {
  ...
  s.render(SafetyState::ARMED, FaultCode::NONE, 0x0001, true, 100, false,
           "OPENER.SHW", 83000);
